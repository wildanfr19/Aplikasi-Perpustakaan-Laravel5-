...
class StatisticsController extends Controller
{
	public function index(Request $request, Builder $htmlBuilder)
	{
		if ($request->ajax()) {
			...
			// $stats = \DB::table('borrow_logs')->join('books','borrow_logs.book_id','=','books.id')
			// 								  ->join('users','borrow_logs.user_id','=','users.id')
			// 								  ->get();
			if ($request->get('filter_status') == 'returned') $stats->returned();
			if ($request->get('filter_status') == 'not-returned') $stats->borrowed();	
